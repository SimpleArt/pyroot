...
from typing import Callable, Iterable, Optional, Sequence, List as list
...
def bisection(
    ...
    if y2 == 0:
        ...
        return _utils.secant(x1, x2, y1, y2)
    elif y2 < 0 < y1:
...
def non_simple(
    ...
    if 0.6 < power < 1.4:
        ...
        return _utils.secant(x1, x2, y1, y2, power)
    ...
    while heap[0]._n < 1024:
        ...
        if box.is_bounded():
            ...
            if all(U - L < abs_err + rel_err * abs(_utils.mean(L, U)) for L, U in zip(sub_box._lower, sub_box._upper)):
                return [_utils.mean(L, U) for L, U in zip(box._lower, box._upper)]
        else:
            heappush(heap, box)
